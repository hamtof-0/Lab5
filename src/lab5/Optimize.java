...
//		nicePrinting(1400, 2000.0D, 0.45D, 0.65D, 0.2D, 0.3D, 42L, 20D, 100);
	...
	private static void nicePrinting(int maxCustomers, double arrivalTime, double gatherMin, double gatherMax,
		...
		System.out.println();
		System.out.println("Minsta antal kassor för minsta antal missade = " + varyLeastCheckouts(maxCustomers,
				arrivalTime, gatherMin, gatherMax, payMin, payMax, seed, closingTime, noChangeFor));
	}
	 ...
	 * @param seed         the seed to create a new run each tim or the same if the
	 *                     same seed is given
	...
	private static int varyLeastCheckouts(int maxCustomers, double arrivalTime, double gatherMin, double gatherMax,
		...
		int leastCheckouts = Integer.MIN_VALUE; // assume the least amount is Integer.Min_value because you cannot get
												// any lower
		...
		while (loopsSinceLastChange < noChangeFor) { // needs to be unchanged for at least 100 runs
			...
			// System.out.println("Working... " + loopsSinceLastChange); // prints how many
			// loops it has done so far
